...
public class CustomRoutingFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (segments.length > 1) {
            ...
            if (serviceId != null) {
                // For the auth service, skip rewriting so that endpoints like /auth/loginCollector remain intact.
                if ("auth".equalsIgnoreCase(prefix)) {
                    log.info("Routing to auth service without rewriting path: {}", originalPath);
                    exchange.getAttributes().put("TARGET_SERVICE", serviceId);
                    return chain.filter(exchange);
                } else if ("client".equalsIgnoreCase(prefix)) {
                    log.info("Routing to client service without rewriting path: {}", originalPath);
                    exchange.getAttributes().put("TARGET_SERVICE", serviceId);
                    return chain.filter(exchange);
                } else if ("users".equalsIgnoreCase(prefix)) {
                    log.info("Routing to users servcices  without rewriting path: {}", originalPath);
                    exchange.getAttributes().put("TARGET_SERVICE", serviceId);
                    return chain.filter(exchange);
                } else if ("transactions".equalsIgnoreCase(prefix)) {
                    log.info("Routing to users transactions  without rewriting path: {}", originalPath);
                    exchange.getAttributes().put("TARGET_SERVICE", serviceId);
                    return chain.filter(exchange);
                } else if ("reports".equalsIgnoreCase(prefix)) {
                    log.info("Routing to users servcices  without rewriting path: {}", originalPath);
                    exchange.getAttributes().put("TARGET_SERVICE", serviceId);
                    return chain.filter(exchange);
                } else if ("loan".equalsIgnoreCase(prefix)) {
                    log.info("Routing to loan servcices  without rewriting path: {}", originalPath);
                    exchange.getAttributes().put("TARGET_SERVICE", serviceId);
                    return chain.filter(exchange);
                } else  {
                    // Remove the prefix from the path so that the internal service sees its expected endpoint.
